...
// ----------------------------------------------------------------
// Helpers — defined FIRST so every hook can call them
// ----------------------------------------------------------------
...
// ================================================================
// Vulkan hook — defined early so InstallHooks can reference it
// ================================================================
...
// ================================================================
// OpenGL hook — wglSwapBuffers (Psych Engine / OpenFL primary)
// ================================================================
...
void InstallHooks()
{
    ...
    // ---- OpenGL (Psych Engine primary) ----
    {
        ...
        if (hGL) {
            ...
            if (pfn &&
                MH_CreateHook(pfn,(void*)&HookedWglSwapBuffers,(void**)&g_origSwapBuffers)==MH_OK)
                MH_EnableHook(pfn);
        }
    }
    ...
    // ---- Vulkan fallback ----
    {
        ...
        if (hVk) {
            ...
            if (pfn &&
                MH_CreateHook(pfn,(void*)&HookedVkPresent,(void**)&g_origVkPresent)==MH_OK)
                MH_EnableHook(pfn);
        }
    }
}
