...
// Custom font path tracking — ImGui loads the TTF directly
static wchar_t g_loadedFontPath[260] = {};

static void LoadCustomFont(const wchar_t* path) {
    // Just track the path — actual loading happens in ImGui init
...
static BOOL WINAPI HookedWglSwapBuffers(HDC hDC)
{
    ...
    if (!g_glInit) {
        ...
        // Load custom TTF font if configured
        ReadConfig();
        if(g_oc.fontPath[0]){
            char fontBuf[260]={};
            ...
            if(!io2.Fonts->AddFontFromFileTTF(fontBuf,sz)){
                io2.Fonts->AddFontDefault(); // fallback
            }
        } else {
